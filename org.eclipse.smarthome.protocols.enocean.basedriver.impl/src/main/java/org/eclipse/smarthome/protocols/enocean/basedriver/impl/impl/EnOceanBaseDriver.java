 ...
 * EnOceanBaseDriver.
 */
public class EnOceanBaseDriver implements EnOceanPacketListener, ServiceTrackerCustomizer<EnOceanDevice,EnOceanDevice>, EventHandler {

	/** TAG */
	public static final String TAG = EnOceanBaseDriver.class.getName();

	private final BundleContext bc;
	private final Hashtable<String,EnOceanDevice> eoDevices;
	private final Hashtable<String, ServiceRegistration<EnOceanHost>> eoHostRefs;

	private final ServiceTracker<EnOceanDevice,EnOceanDevice> eoDevicesTracker;
	private final ServiceTracker<EventAdmin, EventAdmin> eventAdminTracker;
	private final ServiceRegistration<EventHandler> eventHandlerRegistration;
	...
	public EnOceanBaseDriver(BundleContext bundleContext) {
		...
		eoDevices = new Hashtable<String, EnOceanDevice>(10);
		eoHostRefs = new Hashtable<String, ServiceRegistration<EnOceanHost>>(10);

		/* Register initial EnOceanHost */
		// FIXME register listener and allow dynamic add/remove at different ports!
		...
		eventHandlerRegistration = bc.registerService(EventHandler.class, this, ht);
					...
					if (msg.hasTeachInInfo()) {
						Logger.d(TAG,
								"msg has TeachIn info - msg.teachInFunc(): " + msg.teachInFunc()
										...
										+ msg.teachInManuf());
						registerDeviceAndProfile(msg.getSenderId(), msg.getRorg(), msg.teachInFunc(), msg.teachInType(),
								msg.teachInManuf());
					} else {
						Logger.d(TAG,
	...
	public void modifiedService(ServiceReference<EnOceanDevice> reference, EnOceanDevice service) {
		Logger.d(TAG, "IN: EnOceanBaseDriver.modifiedService(ServiceReference: " + reference+ ", Object: " + service + ")");
	}
	
	public void removedService(ServiceReference<EnOceanDevice> reference, EnOceanDevice service) {
		Logger.d(TAG, "IN: EnOceanBaseDriver.removedService(ServiceReference: " + reference + ", Object: " + service + ")");
	}
...
//	}
	 ...
	 */
	public ServiceRegistration<EnOceanHost> registerHost(String hostPath, EnOceanHost enOceanHost) {
	...
	private void unregisterDevices() {
		Iterator<Map.Entry<String, EnOceanDevice>> it = eoDevices.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry<String, EnOceanDevice> entry = it.next();
	...
	private EnOceanDevice getAssociatedDevice(Message msg) {
		...
		String filter = "(&(objectClass=" + EnOceanDevice.class.getName() + ")(" + EnOceanDevice.CHIP_ID + "="
				+ strSenderId + "))";
		ServiceReference<EnOceanDevice>[] ref = null;
		try {
			ref = (ServiceReference<EnOceanDevice>[]) bc.getServiceReferences((String) null, filter);
		} catch (InvalidSyntaxException e) {
	 ...
	 */
	private static <S> ServiceTracker<S,S> registerServiceFrom(BundleContext bundleContext, Class<S> objectClass,
			ServiceTrackerCustomizer<S,S> listener) {
		...
		ServiceTracker<S,S> serviceTracker = new ServiceTracker<S,S>(bundleContext, deviceAdminFilter, listener);
		serviceTracker.open();
		return serviceTracker;
	}

	private static ServiceTracker<EnOceanDevice,EnOceanDevice> registerDeviceListener(BundleContext bundleContext, ServiceTrackerCustomizer<EnOceanDevice,EnOceanDevice> listener)
