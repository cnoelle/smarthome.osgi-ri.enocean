...
public class Activator implements BundleActivator, ServiceTrackerCustomizer, EventHandler {
	...
	public void handleEvent(Event event) {
		Logger.i(TAG, "> handleEvent(event: " + event);
		...
		if (topic.equals(EnOceanEvent.TOPIC_MSG_RECEIVED)) {
			...
			if ("165".equals(rorg)) {
				...
				if ("2".equals(func)) {
					if ("5".equals(type)) {
						Logger.i(TAG, "This event has been sent by an A5-02-05 device.");
						Logger.d(TAG,
				...
				// hex 0xf6 == int 246.
				Logger.i(TAG, "This event has been sent by an F6-wx-yz device.");
				Logger.d(TAG,
				...
				if ("'0x018c0874'".equals(displayId)) {
					...
					if ((new Integer(0x30).byteValue() & 0xff) == statusInHexAsAsAnInt) {
						Logger.i(TAG, "Here, a button has been pressed.");
				...
				// hex 0xd5 == int 213.
				Logger.i(TAG, "This event has been sent by a D5-wx-yz device.");
				Logger.d(TAG,
